/**==== Hashing Implementation: Using Multiplication Hash Function ====**/


/*
 * This program shows the implementation of the hashing using multiplication hash function.
...
int init(int *hashTable, int n)
{
    ...
    {
        scanf("%d", &key);
        if((*(hashTable + multhashFunction(key))) != -1)
        {
            continue;       //Checking if the index already has a value.
        }

        *(hashTable + multhashFunction(key)) = key;
    }
